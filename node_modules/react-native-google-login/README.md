...
`npm i react-native-google-login`

Then follow the [Android guide](docs/android-guide.md) and [iOS guide](docs/ios-guide.md)

if you get error: Execution failed for task ':app:transformClassesWithDexBuilderForDebug'
...
2. Open build.gradel
```
  add this into existing 'defaultConfig' block
     defaultConfig {
        ...
        versionCode *
        versionName "*"
        multiDexEnabled true  <--add this to you existing project
    }
```
## Note
1. You have to create google developer account to register the app name with the SHA1 key
2. You have to generate the google-services.json file from the firebase
## Public API
